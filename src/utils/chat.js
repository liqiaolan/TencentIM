 ...
 */
const onListener = (handleListener) => {
  // 等待 sdk 处于 ready 状态
 ...
 *取消监听事件
 */
const offListener = (handleListener) => {
 ...
 */
const createTextMessageIM = (option) => {
 ...
 * 创建图片消息接口
 */
const createImageMessageIM = (option) => {
 ...
 * 创建音频消息接口
 */
const createAudioMessageIM = (option) => {
  const { targetId, targetType, file } = option;
 ...
 * 创建视频消息接口
 */
const createVideoMessageIM = (option) => {
  const { targetId, targetType, file } = option;
 ...
 * 创建自定义消息接口
 */
const createCustomMessageIM = (option) => {
 ...
 */
const getMessageListIM = async (option) => {
 ...
 */
const sendMessageIM = async (msgType, msgData) => {
  let type = '',
    data = {};
